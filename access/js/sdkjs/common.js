 ;(function () {
    ...
    if (ajaxForm_list.length) {
        Wind.use('ajaxForm', 'artDialog', function () {//wind.use 防止seo阻塞
            if ($.browser.msie) {
...
function open_upload_dialog(dialog_title, callback, extra_params, multi, filetype, app) {
    ...
    app       = app ? app : GV.APP;
    var limit = 0;
    if (extra_params && extra_params.limit !== undefined) {
        limit = extra_params.limit;
    }
    var params = '&multi=' + multi + '&filetype=' + filetype + '&app=' + app + "&limit=" + limit;
    Wind.use("artDialog", "iframeTools", function () {
        art.dialog.open('/admin.php?g=asset&m=asset&a=plupload' + params, {
            title     : dialog_title,
            id        : new Date().getTime(),
            width     : '650px',
            height    : '420px',
